...
function scanDir(dir, options, callback) {
	...
	fs.readdir(dir, function(err, entries) {
		...
		processEntries(dir, entries, options, callback);
	});
}

function processEntries(dir, entries, options, callback) {
	var result = [];
...
function getEmbeddedSitemap(file, options, callback) {
	...
	fs.readFile(file, 'utf8', function(err, contents) {
		...
		var next = function(err, data) {
			...
			var item = sitemap.items.shift();
			if (item instanceof Sitemap.GeneratePI) {
				// instruction to generate sitemap for given dir
				return generateSitemapFromPI(item, file, options, next);
			}

			if (item instanceof SitemapEntry) {
...
function generateSitemapFromPI(pi, origin, options, callback) {
	var prefix = options.prefix;
	if (pi.attributes.prefix) {
		prefix = createUrl(pi.attributes.prefix, prefix, true);
	}
	var dir = path.dirname(origin);
	if (pi.attributes.dir) {
		dir = path.resolve(dir, pi.attributes.dir);
	}
	debug('generate from PI for %s', dir);

	// we should explicitly skip current sitemap.xml from entries
	// in order to avoid inifinite loop
	fs.readdir(dir, function(err, entries) {
		if (err) {
			return callback(err);
		}
		...
		entries = entries.filter(function(entry) {
			return entry !== 'sitemap.xml' || path.join(dir, entry) !== origin;
		});

		processEntries(dir, entries, extend(options, {prefix: prefix}), callback);
	});
}