...
Sitemap.prototype.toXml = function() {
	return '<?xml version="1.0" encoding="UTF-8"?>\n'
		...
		+ this.items.map(function(item) {
			return item instanceof SitemapEntry ? item.toXml('  ') : null;
		}).filter(Boolean).join('\n')
		+ '\n</urlset>\n';
};
...
function GeneratePI(node) {
	var attrs = {};
	if (node) {
		// parse attributes
		node.data.replace(/\s(\w+)=(["'])(.*?)\2/g, function(str, name, quote, value) {
			attrs[name] = value;
		});
	}
...
module.exports.GeneratePI = GeneratePI;
...
function parseUrlsetNode(node) {
	return node.children.reduce(function(out, child) {
		...
		} else if (child.type === 'directive' && /^\??generate\-sitemap\b/.test(child.name)) {
			out.push(new GeneratePI(child));
		}
		return out;
	}, []);
}
